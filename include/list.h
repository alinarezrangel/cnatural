...
#if !defined(H_CNATURAL_LIST_FUNCTIONS_H_)
#define H_CNATURAL_LIST_FUNCTIONS_H_ 1
...
typedef struct cnatural_list
{
	...
	*/
	struct cnatural_list* back;

	/**
	* @brief The node value.
	*
	* You can set it with cnatural_list_set_at() or get it with
	* cnatural_list_get_at().
	*/
	void* value;
} cnatural_list;
...
*/
int cnatural_list_create(cnatural_list** node);
...
* @return 0 on sucess, -1 on failure or 1 if the action cannot be done.
*/
int cnatural_list_destroy(cnatural_list** node);
...
* so you should call cnatural_list_destroy() in this node.
*
...
*
* Equivalent to call cnatural_list_remove() and
* cnatural_list_destroy().
*
...
*
* It's like call cnatural_list_remove() on the first element.
*
* @param head The head of the list.
* @param node Pointer where the removed node will be.
* @return 0 on sucess, -1 on failure or 1 if the action cannot be done.
*/
int cnatural_list_pop_front(
	cnatural_list* head,
	cnatural_list** node
);
...
*
* It's like call cnatural_list_remove() on the last element.
*
* @param head The head of the list.
* @param node Pointer where the removed node will be.
* @return 0 on sucess, -1 on failure or 1 if the action cannot be done.
*/
int cnatural_list_pop_back(
	cnatural_list* head,
	cnatural_list** node
);
...
* @return 0 on sucess, -1 on failure or 1 if the action cannot be done.
*/
int cnatural_list_size(cnatural_list* head, size_t* size);
...
#endif /* ~H_CNATURAL_LIST_FUNCTIONS_H_ */
